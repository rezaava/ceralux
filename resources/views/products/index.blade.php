...
<style>
    .product-card {height: 300px;background-size: cover;background-position: center;margin-bottom: 10px;display: flex;align-items: center;justify-content: center;position: relative;border-radius: 5px;color: white;overflow: hidden;transition: transform 0.3s ease, box-shadow 0.3s ease;}
    .product-card:hover {transform: scale(1.03);box-shadow: 0 12px 24px rgba(0, 0, 0, 0.2);}
    .product-overlay {background: rgba(0, 0, 0, 0.5);position: absolute;border-radius: 5px;inset: 0;transition: opacity 0.3s ease;}
    .product-card:hover .product-overlay {opacity: 0.7;}
    .product-content {position: relative;text-align: center;z-index: 2;}
    .product-btn {background-color: transparent !important;color: #d0bc7e !important;border: 2px solid #d0bc7e !important;transition: all 0.3s ease;position: relative;overflow: hidden;}
    ...
    .product-name {font-family: 'Vazirmatn', sans-serif;color: #262f40;margin-top: 10px;}
    [dir="rtl"] .product-name {text-align: right;}
    [dir="ltr"] .product-name {text-align: left;}
    .size-title {color: #d0bc7e;font-family: 'Vazirmatn', sans-serif;margin-bottom: 30px;}
    .product-card {opacity: 0;transform: translateY(20px);animation: fadeUp 0.5s ease forwards;animation-delay: calc(0.1s * var(--aos-index));}
        ...
        <div class="col-md-3 mb-4" data-aos="zoom-in" style="--aos-index: {{ $i % 8 + 1 }};">
            <div class="product-card"
                ...
                <div class="product-content">
                    <a id="p{{$i}}" href="/products/info/{{$product->id}}" class="btn product-btn">{{ __('messages.product_info') }}</a>
                </div>
            </div>
